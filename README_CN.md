...
UDP 的 `DeviceInfo` 仅提供报文中携带的 `protocolVersion`。UDP 帧不提供设备
身份与版本字符串、温度、状态码、厂商数据或 EEPROM 数据，因此这些字段保持为空。
...
- 串口时间戳（包括检测到周期性 PPS 回绕的情况）以首个有效帧的主机接收时间锚定到 Unix 纪元，并按设备原始毫秒增量展开。它们处于统一时间域，但 SDK 当前不保证 PPS 边沿与主机 Unix 整秒严格对齐，也不消除传输和调度延迟。没有 PPS 时，递增的设备计数同样只是主机时间估算值，不代表设备时钟已经同步。
- UDP 只有在打开后的第一帧有效数据构成且与主机系统时间相差不超过 5 秒的 Unix 纪元时间时才直接使用该时间域；同一次打开期间不会切换时间域。`nanoseconds` 字段不在 `[0, 999,999,999]` 范围内的报文会被拒绝；已经通过解析但没有推进当前会话时间线的时间戳会进行单调递增处理。因此未同步或过期的设备时钟会使用主机锚定的相对时间估算。
